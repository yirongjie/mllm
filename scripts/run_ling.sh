...
adb push ../models/ling-lite-base-1.5-kai_q4_0.mllm /data/local/tmp/mllm/models/
# if push failed, exit
if [ $? -ne 0 ]; then
    echo "adb push failed"
    exit 1
fi
adb shell "cd /data/local/tmp/mllm/bin && ./demo_bailing_moe"
# adb shell "cd /data/local/tmp/mllm/bin && ./demo_bailing_moe_mbp"