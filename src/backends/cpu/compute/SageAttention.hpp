...
template <> inline float to_float<float>(float val) { return val; }
template <> inline float to_float<mllm_fp16_t>(mllm_fp16_t val) { return MLLM_FP16_TO_FP32(val); }

#ifdef __AVX2__
inline float _mm256_hmax_ps(__m256 x) { __m128 lo = _mm256_castps256_ps128(x); __m128 hi = _mm256_extractf128_ps(x, 1); __m128 max_val = _mm_max_ps(lo, hi); max_val = _mm_max_ps(max_val, _mm_shuffle_ps(max_val, max_val, _MM_SHUFFLE(0, 0, 2, 2))); max_val = _mm_max_ps(max_val, _mm_shuffle_ps(max_val, max_val, _MM_SHUFFLE(0, 0, 0, 1))); return _mm_cvtss_f32(max_val); }
inline float hsum_ps(__m256 v) { __m128 vlow = _mm256_castps256_ps128(v); __m128 vhigh = _mm256_extractf128_ps(v, 1); vlow = _mm_add_ps(vlow, vhigh); __m128 shuf = _mm_movehdup_ps(vlow); __m128 sums = _mm_add_ps(vlow, shuf); shuf = _mm_movehl_ps(shuf, sums); sums = _mm_add_ss(sums, shuf); return _mm_cvtss_f32(sums); }
inline int32_t hsum_i32(__m256i v) { __m128i vlo = _mm256_castsi256_si128(v); __m128i vhi = _mm256_extracti128_si256(v, 1); __m128i vsum = _mm_add_epi32(vlo, vhi); vsum = _mm_add_epi32(vsum, _mm_shuffle_epi32(vsum, _MM_SHUFFLE(1, 0, 3, 2))); vsum = _mm_add_epi32(vsum, _mm_shuffle_epi32(vsum, _MM_SHUFFLE(2, 3, 0, 1))); return _mm_cvtsi128_si32(vsum); }

inline __m256 load_and_convert_to_fp32_vec(const float* ptr) { return _mm256_loadu_ps(ptr); }
#ifdef __F16C__
inline __m256 load_and_convert_to_fp32_vec(const mllm_fp16_t* ptr) { return _mm256_cvtph_ps(_mm_loadu_si128((const __m128i*)ptr)); }
#endif
#elif __ARM_NEON__
inline float _vmaxvq_f32_hmax(float32x4_t x) { return vmaxvq_f32(x); }
inline void load_and_convert_to_fp32x4x2(const float* ptr, float32x4_t& out_lo, float32x4_t& out_hi) { out_lo = vld1q_f32(ptr); out_hi = vld1q_f32(ptr + 4); }
#if defined(__ARM_FP16_FORMAT_IEEE)
inline void load_and_convert_to_fp32x4x2(const mllm_fp16_t* ptr, float32x4_t& out_lo, float32x4_t& out_hi) { float16x8_t v_f16 = vld1q_f16(reinterpret_cast<const float16_t*>(ptr)); out_lo = vcvt_f32_f16(vget_low_f16(v_f16)); out_hi = vcvt_f32_f16(vget_high_f16(v_f16)); }
#endif
#endif

inline void aligned_alloc(void **ptr, size_t required_bytes, size_t align) { if (align % sizeof(void *) != 0 || (align & (align - 1)) != 0) { *ptr = nullptr; return; } if (posix_memalign(ptr, align, required_bytes) != 0) { *ptr = nullptr; } }
...
inline void quantize_row_simd(const float* float_row, int8_t* int8_row, float* scale, int dim_size, float sm_scale, float* temp_buf) {
    ...
    float max_abs_val = 0.0f;
    #if defined(__AVX2__)
        __m256 max_vec = _mm256_setzero_ps(); const __m256 abs_mask = _mm256_castsi256_ps(_mm256_set1_epi32(0x7FFFFFFF));
        int d = 0;
        ...
        float32x4_t max_vec = vdupq_n_f32(0.0f); int d = 0;
...
#if defined(__ARM_NEON__) && defined(__ARM_FP16_FORMAT_IEEE)
...
public:
    ...
    ~WorkspaceManager() { for (auto & ptr : workspace_) { if (ptr) aligned_free(ptr); } }
    void** get_workspace(const std::vector<size_t>& required_sizes) {
        if (workspace_.empty()) { workspace_.resize(required_sizes.size(), nullptr); current_sizes_.resize(required_sizes.size(), 0); }
...
struct SAGE_CPU_IMPL {
    ...
    void configure(int32_t Br_, int32_t Bc_, int32_t Q_Head_, int32_t KV_Head_, int32_t threads_) { Br = Br_; Bc = Bc_; Q_Head = Q_Head_; KV_Head = KV_Head_; threads = threads_; }
    void init_workspace(void** workspace) {
        acc_o_ = static_cast<float*>(workspace[0]); acc_s_ = static_cast<float*>(workspace[1]);
        logsum_ = static_cast<float*>(workspace[2]); scoremax_ = static_cast<float*>(workspace[3]);
        scoremax_prev_ = static_cast<float*>(workspace[4]); score_scale_ = static_cast<float*>(workspace[5]);
        score_sum_ = static_cast<float*>(workspace[6]); q_quant_tile_ = static_cast<int8_t*>(workspace[7]);
        k_quant_tile_ = static_cast<int8_t*>(workspace[8]); q_scale_ = static_cast<float*>(workspace[9]);
    ...
    void init_temp(float* logsum, float* scoremax, float* acc_o, int Br_fixed, int dim_size) { for(int i=0; i<Br_fixed; ++i) { logsum[i] = 0.0f; scoremax[i] = NEG_INF; } memset(acc_o, 0, Br_fixed * dim_size * sizeof(float)); }
    ...
    void quantize_and_mma0_sdot(int Br_fixed, int Bc_fixed, const int8_t* q_quant_tile, const int8_t* k_quant_tile, float* acc_s, const float* q_scale, const float* k_scale, int dim_size, int global_r_start, int global_c_start, bool causal) {
    #if defined(__ARM_NEON__) && defined(__ARM_FEATURE_DOTPROD)
        constexpr int MR = 4;
        constexpr int NR = 4;
        ...
        for (; r < br_4_end; r += MR) {
            ...
            for (; c < bc_4_end; c += NR) {
                ...
                for(int i=0; i<MR; ++i) for(int j=0; j<NR; ++j) vacc[i][j] = vdupq_n_s32(0);
                ...
                int d = 0;
                for (; d <= dim_size - 16; d += 16) {
                    int8x16_t k0 = vld1q_s8(k_ptr[0] + d); int8x16_t k1 = vld1q_s8(k_ptr[1] + d);
                    int8x16_t k2 = vld1q_s8(k_ptr[2] + d); int8x16_t k3 = vld1q_s8(k_ptr[3] + d);
                    for(int i=0; i<MR; ++i) {
        ...
        for (r = 0; r < Br_fixed; ++r) {
            ...
            for (int c = start_c; c < Bc_fixed; ++c) {
                if (causal && (global_c_start + c) > (global_r_start + r)) { acc_s[r * Bc + c] = NEG_INF; continue; }
        ...
        for (int r = 0; r < Br_fixed; ++r) {
            for (int c = 0; c < Bc_fixed; ++c) {
                ...
                for (; d <= dim_size - 32; d += 32) {
                    __m256i q_i8_v = _mm256_loadu_si256((const __m256i*)(q_quant_line + d));
                    __m256i k_i8_v = _mm256_loadu_si256((const __m256i*)(k_quant_line + d));
                    // maddubs(a,b) computes a(unsigned)*b(signed)
                    // We can treat one as unsigned and the other as signed.
                    // Or use VPMADDSWD which treats both as signed 16-bit words.
                    // Here we use VPMADDUBSW, requires one operand to be interpreted as unsigned.
                    __m256i prod_i16_v = _mm256_maddubs_epi16(q_i8_v, k_i8_v); 
                    // madd_epi16 horizontally adds adjacent pairs of 16-bit integers
                    // and accumulates them into a 32-bit integer.
                    // Here we multiply by a vector of 1s to achieve this accumulation.
                    __m256i ones_i16_v = _mm256_set1_epi16(1);
                    __m256i sum_i32_v = _mm256_madd_epi16(prod_i16_v, ones_i16_v);
                    acc_i32_v = _mm256_add_epi32(acc_i32_v, sum_i32_v);
                }
    ...
    void softmax(int Br_fixed, int Bc_fixed, float* acc_s, float* scoremax, float* scoremax_prev, float* score_scale, float* score_sum, float* logsum) {
        ...
        for(int r=0; r<Br_fixed; ++r){ float* row = acc_s + r*Bc; float current_max = scoremax[r]; for(int c=0; c<Bc_fixed; ++c) current_max = std::max(current_max, row[c]); scoremax[r] = current_max; }
        for(int r=0; r<Br_fixed; ++r) score_scale[r] = expf(scoremax_prev[r] - scoremax[r]);
        for(int r=0; r<Br_fixed; ++r){ float* row = acc_s + r*Bc; float sm = scoremax[r]; float sum = 0.f; for(int c=0; c<Bc_fixed; ++c){ if(row[c] > NEG_INF / 2) { float val = expf(row[c] - sm); row[c] = val; sum += val; } else { row[c] = 0.f; } } score_sum[r] = sum; }
    ...
    void rescale(int Br_fixed, float* acc_o, const float* score_scale, int dim_size) {
        for(int r=0; r<Br_fixed; ++r){ float scale_val = score_scale[r]; float* row_ptr = acc_o + r * dim_size;
            #if defined(__AVX2__)
    ...
    void scale_and_store(int Br_fixed, const float* acc_o, const float* logsum, const float* v_mean, float* O, int head_size, int dim_size) {
        ...
        for(int r=0; r<Br_fixed; ++r){ float inv_logsum = (logsum[r] > 1e-9f) ? 1.f / logsum[r] : 0.f; const float* o_row = acc_o + r * dim_size; float* O_row = O + r * o_stride;
                ...
                int d = 0;
                for(; d <= dim_size - 4; d+=4){ float32x4_t o_vec = vld1q_f32(o_row + d); float32x4_t vm_vec = vld1q_f32(v_mean + d); vst1q_f32(O_row + d, vfmaq_f32(vm_vec, o_vec, inv_logsum_vec)); }
                for(; d < dim_size; ++d) O_row[d] = o_row[d] * inv_logsum + v_mean[d];
            #else
...
void sage_attention_forward_cpu_dispatch(const float* Q, const KVDtype* K, const KVDtype* V, float* O, int32_t batch_size, int32_t q_head, int32_t kv_head, int32_t seq_size_q, int32_t seq_size_k, int32_t dim_size, bool causal_mask, int32_t threads, int32_t br, int32_t bc) {
    ...
    if (seq_size_q > 1) { // Prefill
        ...
        const std::vector<size_t> required_sizes = { 
            (size_t)threads*decode_br*dim_size*sizeof(float), (size_t)threads*decode_br*bc*sizeof(float), 
            (size_t)threads*decode_br*sizeof(float), (size_t)threads*decode_br*sizeof(float),
            (size_t)threads*decode_br*sizeof(float), (size_t)threads*decode_br*sizeof(float),
            (size_t)threads*decode_br*sizeof(float), (size_t)threads*decode_br*dim_size*sizeof(int8_t),
            (size_t)batch_size*kv_head*seq_size_k*dim_size*sizeof(int8_t),
            (size_t)threads*decode_br*sizeof(float), (size_t)batch_size*kv_head*seq_size_k*sizeof(float),
            (size_t)threads*dim_size*sizeof(float), (size_t)threads*dim_size*sizeof(float),
            (size_t)threads*dim_size*sizeof(float),
        };
        void **workspace = manager.get_workspace(required_sizes);
    ...
    for (int b = 0; b < batch_size; ++b) {
        for (int h = 0; h < kv_head_size; ++h) {
            ...
            for (int s = 0; s < seq_size_k; ++s) {
                ...
                // Call the new per-group quantization function
                quantize_row_per_group_simd(thread_smoothed_buf, target_k_quant + s * dim_size, target_k_scale + s * num_k_blocks, dim_size, 1.0f, thread_sum_buf);
            }
        }
    }
}
        ...
        for (int r = 0; r < Br_fixed; ++r) {
            for (int c = 0; c < Bc_fixed; ++c) {
                ...
                for (int g = 0; g < num_k_blocks; ++g) {
                    ...
                    int d = 0;
                    // Process 32 bytes at a time
                    for (; d <= QK_K_BLOCK_SIZE - 32; d += 32) {
                        __m256i q_i8_v = _mm256_loadu_si256((const __m256i*)(q_group_ptr + d));
                        __m256i k_i8_v = _mm256_loadu_si256((const __m256i*)(k_group_ptr + d));
...
void sage_attention_forward_cpu_dispatch(const float* Q, const KVDtype* K, const KVDtype* V, float* O, int32_t batch_size, int32_t q_head, int32_t kv_head, int32_t seq_size_q, int32_t seq_size_k, int32_t dim_size, bool causal_mask, int32_t threads, int32_t br, int32_t bc) {
    ...
    // Ensure dim_size is divisible by the block size
    ...
    if (seq_size_q > 1) { // Prefill
        const std::vector<size_t> required_sizes = { 
            ...
            (size_t)threads * br * dim_size * sizeof(int8_t),
            (size_t)batch_size * kv_head * seq_size_k * dim_size * sizeof(int8_t),
            (size_t)threads * br * num_k_blocks * sizeof(float), // q_scale_ (increased size)
            (size_t)batch_size * kv_head * seq_size_k * num_k_blocks * sizeof(float), // k_scale_ (increased size)
            (size_t)threads * dim_size * sizeof(float),
            (size_t)threads * dim_size * sizeof(float),
            (size_t)threads * dim_size * sizeof(float),
        };
        ...
        const std::vector<size_t> required_sizes = { 
            ...
            (size_t)threads*decode_br*sizeof(float), (size_t)threads*decode_br*dim_size*sizeof(int8_t),
            (size_t)batch_size*kv_head*seq_size_k*dim_size*sizeof(int8_t),
            (size_t)threads*decode_br * num_k_blocks * sizeof(float), // q_scale_ (increased size)
            (size_t)batch_size*kv_head*seq_size_k * num_k_blocks * sizeof(float), // k_scale_ (increased size)
            (size_t)threads*dim_size*sizeof(float), (size_t)threads*dim_size*sizeof(float),
            (size_t)threads*dim_size*sizeof(float),
        };
