...
// 新的命名空间，专门用于零拷贝的Q8 KV Cache Attention
...
#elif defined(__ARM_NEON__) && defined(__ARM_FEATURE_DOTPROD)
                    int32x4_t acc0 = vdupq_n_s32(0);
                    int32x4_t acc1 = vdupq_n_s32(0);
                    int32x4_t acc2 = vdupq_n_s32(0);
                    int32x4_t acc3 = vdupq_n_s32(0);
                    int d = 0;
                    for (; d <= QK_K_BLOCK_SIZE - 64; d += 64) {
                        acc0 = vdotq_s32(acc0, vld1q_s8(q_group_ptr + d + 0), vld1q_s8(k_group_ptr + d + 0));
                        acc1 = vdotq_s32(acc1, vld1q_s8(q_group_ptr + d + 16), vld1q_s8(k_group_ptr + d + 16));
                        acc2 = vdotq_s32(acc2, vld1q_s8(q_group_ptr + d + 32), vld1q_s8(k_group_ptr + d + 32));
                        acc3 = vdotq_s32(acc3, vld1q_s8(q_group_ptr + d + 48), vld1q_s8(k_group_ptr + d + 48));
                    }
                    g_dot = vaddvq_s32(acc0) + vaddvq_s32(acc1) + vaddvq_s32(acc2) + vaddvq_s32(acc3);
                    for (; d < QK_K_BLOCK_SIZE; ++d) g_dot += q_group_ptr[d] * k_group_ptr[d];
#else
                    for (int d = 0; d < QK_K_BLOCK_SIZE; ++d) g_dot += q_group_ptr[d] * k_group_ptr[d];
#endif
    ...
    void mma1(int Br_f, int Bc_f, const int8_t *p_quant_block, const float *p_scale_block, const block_q8_0f *v_cache_tile, float *acc_o, int D) {
        ...
        for (int r = 0; r < Br_f; ++r) {
            ...
            for (int c = 0; c < Bc_f; ++c) {
                ...
                for (int g = 0; g < num_v_blocks; ++g) {
                    const int g_start = g * QK_K_BLOCK_SIZE;
                    const float32x4_t v_scale_vec = vdupq_n_f32(v_block_line[g].scale);
                    const int8_t *v_qs = v_block_line[g].qs;
                    int d = 0;
                    for (; d <= QK_K_BLOCK_SIZE - 8; d += 8) {
                        int8x8_t v_i8 = vld1_s8(v_qs + d);
                    ...
                    for (; d < QK_K_BLOCK_SIZE; ++d) {
                        o_row[g_start + d] += p_dequant_val * ((float)v_qs[d] * v_block_line[g].scale);
                    }
                }
#else
                // Scalar fallback
...
#pragma omp parallel for num_threads(threads) schedule(dynamic, 1)
        for (int32_t b_h = 0; b_h < batch_size * head_size; ++b_h) {
            ...
            // [NEW] Get thread-local pointers for quantized P
