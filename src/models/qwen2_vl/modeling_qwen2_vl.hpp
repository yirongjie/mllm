...
public:
    ...
    QWen2Attention(const Qwen2VLConfig &config, const QWenNameConfig &names, const string &base_name) {
        ...
        k_cache = KVCache(num_key_value_heads, head_dim, num_key_value_groups, config.cache_limit, (config.attn_implementation == "flash_attention_2" || config.attn_implementation == "sage_attention"), base_name + "k_cache");
        v_cache = KVCache(num_key_value_heads, head_dim, num_key_value_groups, config.cache_limit, (config.attn_implementation == "flash_attention_2" || config.attn_implementation == "sage_attention"), base_name + "v_cache");
...
public:
    ...
    void get_position_ids(vector<Tensor> &inputs) {
        if (inputs[0].sequence() > 1) {
            ...
            auto rope_indices = get_rope_index(inputs[0], inputs[2], video_grid_thw);
